...
import { QUEST_TYPES, type QuestType } from "@/lib/quest-types";
...
export async function POST(req: Request) {
  try {
    ...
    const completed = await supabaseAdmin
      .from("quests")
      .select("title, quest_type, created_at")
      .eq("user_id", userId)
      .eq("completed", true)
    ...
    for (const t of QUEST_TYPES) counts.set(t, 0);
    for (const r of rows) {
      const t = r.quest_type as QuestType;
      counts.set(t, (counts.get(t) ?? 0) + 1);
    }

    const stats: QuestTypeCount[] = QUEST_TYPES.map((t) => ({
      quest_type: t,
